...
public class ChromeDevtoolsServer implements SimpleEndpoint {
    ...
    public void onMessage(SimpleSession simpleSession, String str) {
        ...
        try {
            ...
            if (LogRedirector.isLoggable(str2, 2)) {
                ...
                sb2.append(e);
                LogRedirector.v(str2, sb2.toString());
            }
            ...
            sb3.append(e2);
            LogRedirector.i(str2, sb3.toString());
            closeSafely(simpleSession, 1011, e2.getClass().getSimpleName());
        } catch (JSONException e3) {
            LogRedirector.v(str2, "Unexpected JSON exception processing message", e3);
    ...
    private static void logDispatchException(JsonRpcException jsonRpcException) {
        ...
        int i = AnonymousClass1.$SwitchMap$com$facebook$stetho$inspector$jsonrpc$protocol$JsonRpcError$ErrorCode[errorMessage.code.ordinal()];
        String str = TAG;
        if (i != 1) {
            LogRedirector.w(str, "Error processing remote message", jsonRpcException);
            return;
        }
        StringBuilder sb = new StringBuilder();
