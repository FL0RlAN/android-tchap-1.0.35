...
class Frame {
    ...
    private long decodeLength(byte b, InputStream inputStream) throws IOException {
        ...
        if (b == 126) {
            return (long) (((readByteOrThrow(inputStream) & UByte.MAX_VALUE) << 8) | (readByteOrThrow(inputStream) & UByte.MAX_VALUE));
        }
        if (b == Byte.MAX_VALUE) {
            long j = 0;
            for (int i = 0; i < 8; i++) {
                j = (j | ((long) (readByteOrThrow(inputStream) & UByte.MAX_VALUE))) << 8;
            }
            return j;
        }
